...
exports.signup = (req, res) => {
    ...
    .catch(err => {
        ...
        if(err.code === "auth/email-already-in-use"){
            return res.status400().json({ email: 'Email is already in use'})
        } else {
            return res.status(500).json({ general: 'Something went wrong, plz try again' });
        }
    });
}
